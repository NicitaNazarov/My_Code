class InfoMessage:
    ...
    def get_message(self):
        return (f'Тип тренировки: {self.training_type}; Длительность: {(self.time * 60):.1f} минут.;'
                f' Дистанция: {self.distance:.3f} км;'
                f'Ср. скорость: {self.speed} км/ч; Потрачено ккал: {self.calories:.1f}')
...
class Training:
    ...
    def get_distance(self) -> float:
        return self.action * self.LEN_IN_STEP / M_IN_KM
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.CALORIES_MEAN_SPEED_MULTIPLIER * self.weight + (self.get_mean_speed()**2 / self.height/ self.HEIGHT_IND)
...
def read_package(training_type: str, data: list) -> Training:
     ...
     if training_type not in training_classes:
         return (f'Ошибка, неизвестный вид тренировки: {training_type}')
     
     return training_classes[training_type](*data)
